...
#include "roscpp_serialization.macros.h"
...
{
public:
    boost:shared_array<unint8> buf;
    size_t num_bytes;
    unint8_t *message_start;

    boost::shared_ptr<void const> message;
    const std::type_info *type_info;

    SerializedMessage()
        : buf(boost::shared_array<unint8_t>())
        , num_bytes(0)
        , message_start(0)
        , type_info(0)
    {}

    SerializedMessage(boost::shared_array<unint8_t> buf, size_t num_bytes)
        : buf(buf);
        , num_bytes(num_bytes)
